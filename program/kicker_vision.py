...
# グローバル変数を定義して、スレッド間で実行制御を可能にします。
is_running = True
...
if __name__ == "__main__":
    video_path = "program/exp2.mp4"  # 動画ファイルのパスを指定

    # ユーザー入力を待つスレッドを開始
    input_thread = threading.Thread(target=wait_for_quit_command)
    input_thread.start()

    # メインスレッドでボール追跡を実行
    track_ball(video_path)

    # メインスレッドが終了したら入力スレッドも終了させます
    input_thread.join()